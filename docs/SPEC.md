...
| 画面 | 配置 |
| --- | --- |
| 縦長 (〜759px) | ツールバー → プレビュー → 説明・範囲一覧 の縦積み |
| 横長・PC (760px〜) | 左にプレビュー (高さいっぱい)、右にツールバーと各パネル |

横長では見出しとフッターを畳み、プレビューを高さ基準で拡大する。
横長のスマホ (高さ560px以下) ではナビゲーションレールもアイコンのみに縮める。
レール自体が画面より高くなると、どう詰めてもページ全体が縦スクロールしてしまうため。
...
- **v0.4 (現在)** — 書き出しを画面上部へ移動、クリアをアプリバーに一本化、
  横長の画面向けの1画面レイアウト、背の低い画面でのナビゲーション圧縮
